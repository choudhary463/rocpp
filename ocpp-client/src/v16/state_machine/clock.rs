...
use crate::v16::{cp::core::ChargePointCore, drivers::{database::Database, hardware_interface::{HardwareBridge, HardwareInterface}}};
...
impl Instant {
    ...
    pub fn now<H: HardwareInterface>(hw: &HardwareBridge<H>) -> Self {
...
impl<D: Database, H: HardwareInterface> ChargePointCore<D, H> {
    pub(crate) fn get_time(&self) -> Option<DateTime<Utc>> {
        self.base_time
            .map(|(base_dt, base_instant)| base_dt + Duration::from_micros(Instant::now(&self.hw).duration_since(base_instant).0 as u64))
    }
    ...
    pub(crate) fn set_time(&mut self, dt: DateTime<Utc>) {
        ...
        self.base_time = Some((dt, Instant::now(&self.hw)));
