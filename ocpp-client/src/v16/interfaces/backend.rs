use alloc::{vec, format, vec::Vec, string::{String, ToString}, collections::{BTreeSet, BTreeMap}};

use rocpp_core::v16::{messages::{reserve_now::ReserveNowRequest, status_notification::StatusNotificationRequest}, types::{AvailabilityType, ChargePointErrorCode, IdTagInfo, Measurand}};

use crate::v16::state_machine::{auth::LocalListChange, connector::ConnectorState, firmware::FirmwareInstallStatus, transaction::{MeterValueLocal, TransactionEvent}};
...
impl<I: ChargePointInterface> ChargePointBackend<I> {
    ...
    pub async fn get_start_stop_meter_value(&mut self, connector_id: usize) -> u64 {
        self.interface.get_meter_value(
            connector_id,
            &MeterDataType {
    ...
    pub async fn db_change_operative_state(&mut self, connector_id: usize, state: AvailabilityType) {
        let key = connector_id.to_string();
        let value = serde_json::to_string(&state).unwrap();
        self.interface
            .db_transaction("availabilitytype", vec![(key.as_str(), Some(value.as_str()))]).await;
    }
    pub async fn db_get_connector_state(&mut self, num_connectors: usize,) -> (Vec<ConnectorState>, Vec<StatusNotificationRequest>) {
    ...
    pub async fn db_update_config(&mut self, key: &str, value: &str) {
        self.interface
            .db_transaction("config", vec![(key, Some(value))]).await;
    }
    pub async fn db_get_from_cache(&mut self, id_tag: &str) -> Option<IdTagInfo> {
        self.interface.db_get("cache", &id_tag).await.map(|s| serde_json::from_str(s).unwrap())
    }
    pub async fn db_update_cache(&mut self, id_tag: &str, info: IdTagInfo) {
        let value = serde_json::to_string(&info).unwrap();
        self.interface
            .db_transaction("cache", vec![(id_tag, Some(value.as_str()))]).await;
    } 
    ...
    pub async fn db_get_from_local_list(&mut self, id_tag: &str) -> Option<IdTagInfo> {
        self.interface.db_get("local_list", &id_tag).await.map(|s| serde_json::from_str(s).unwrap())
    }
    ...
    pub async fn db_get_local_list_version(&mut self) -> Option<i32> {
        self.interface.db_get("local_list", "version#").await.map(|t| t.parse().unwrap())
    }
    ...
    pub async fn db_change_firmware_state(&mut self, state: FirmwareInstallStatus) {
        ...
        let value = serde_json::to_string(&state).unwrap();
        self.interface
            .db_transaction("firmware", vec![(key, Some(value.as_str()))]).await;
    }
    pub(crate) async fn db_add_reservation(&mut self, reservation: ReserveNowRequest) {
        ...
        self.interface
            .db_transaction("reservation", vec![(key.as_str(), Some(value.as_str()))]).await;
    }
    ...
    pub(crate) async fn db_get_transaction_event(&mut self, index: u64) -> TransactionEvent {
        let val = self.interface.db_get("transaction", format!("event:{}", index).as_str()).await.unwrap();
        serde_json::from_str(val).unwrap()
    }

    pub(crate) async fn db_get_all_stop_meter_val(&mut self, local_transaction_id: u32, len: usize) -> Vec<MeterValueLocal> {
        let mut res = Vec::new();
        for index in 0..len {
            let val = self.interface.db_get("transaction", format!(
                "meter:{}:{}",
                local_transaction_id, index
            ).as_str()).await.unwrap();
    ...
    pub(crate) async fn db_add_stop_meter_val(&mut self, local_transaction_id: u32, index: usize, value: MeterValueLocal)  {
        let key = format!("meter:{}:{}", local_transaction_id, index);
        let value = serde_json::to_string(&value).unwrap();
        self.interface.db_transaction("transaction", vec![(key.as_str(), Some(value.as_str()))]).await;
    }
