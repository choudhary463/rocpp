...
impl TimeoutService {
    ...
    pub fn add_or_update(&mut self, id: TimerId, when: Instant) {
    ...
    pub fn next_deadline(&self) -> Option<(Instant, TimerId)> {
        self.deadline_queue.iter().next().map(|(k, v)| (*k, *v))
    }
}

impl Stream for TimeoutService {
    type Item = TimerId;
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        self.needs_reschedule = true;
        self.remove(expired_timer_id);
        Poll::Ready(Some(expired_timer_id))
    }
}
